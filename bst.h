...
//    Title:            node Class
//
//    Description:		This file contains the class definition for node
//
//    Programmer:		Originally written by Paul Bladek
//						
//   
//    Date of use:		6/6/2016
// 
//    Version:          1.0
//  
...
//	
//		inline:	
//			node(T d = 1){} -- constructor of the avltree
...
//			node<T>& operator=(const node<T>& n) -- overloaded =, throw bad alloc
//			int setHeight() -- recursively sets the Height of the node
//		
//		private:
//			T m_data;
//			int m_height;
...
namespace RD_BST
{
	...
	//    Title:            bst Class
	//
	//    Description:		This file contains the class definition for bst
	//
	//    Programmer:		Originally written by Paul Bladek
	//						
	//   
	...
	//	
	//		inline:	
	//			bst() :root(nullptr), parentptr(&root){} -- constructor
	...
	//			void printXlevel(ostream& out)const
	//			int getHeight()const
	//			void setHeight()
	//			~bst() -- destructor
	//
	//		non-inline:
	//			bst<T>& operator=(const bst<T>& t) -- overloaded =
	//			bst<T>& operator+=(const bst<T>& t) -- overloaded +=
	//			void addtree(const node<T>* np) -- recursively adds in 
	//											the contents of a second tree
	//			void findfirst(const T& d, node<T>* &np, node<T>* &match) -- recursively 
	//									finds the first occurance of a data item
	//			void insert(T d) -- inserts a new element
	//			void insert(T d, node<T>* &cur) -- inserts a new element into the tree
	//			void print(node<T>* cur, ostream& out)const -- recursively prints out
	//															the tree inorder
	//			void setLevel(node<T>* cur, vector<T>& levelVector,
	//					int level2print, int position) const -- recursuively sets 
	//									levelVector to represent the specified level
	//			void printXlevel(node<T>* cur, ostream& out) const -- prints out the tree 
	//																in X level order
	//			void deltree(node<T>* &cur) --recursively deletes out the subtree
	//			T popnode(node<T>* &cur) -- pops a given node
	//			T poplow(node<T>* &cur)  -- pops out the leftmost child of cur
	//			T pophigh(node<T>* &cur) -- pops out the rightmost child of cur
	//			T popfirst(const T& d, node<T>* np) -- pops first node matching d
	//			int getNumberOfNodes()const -- returns the number of nodes in the tree
	//			int getNumberOfNodes(node<T>* np)const -- returns the number of nodes in the tree
	//			ostream& operator<<(ostream& out, bst<T> tree) -- Overloaded << for bst<T>
	//			
	//		
	//		protected:
